---
title: "MANIFEST.json field reference for GPT skill packages"
sidebarTitle: "Manifest Reference"
description: "Complete MANIFEST.json field reference for GPT Project Skill System packages: required fields, load behavior, file categories, and validation rules."
---
...
<CardGroup cols={2}>
  <Card title="Missing required field" icon="circle-xmark" href="/building-skills/manifest-reference">
    Either `skill_name` or `version` is absent from the manifest.
  </Card>
  <Card title="Invalid skill_name" icon="circle-xmark" href="/building-skills/manifest-reference">
    `skill_name` does not match `^[a-z0-9_-]+$`, is empty, or does not match the ZIP filename, command slug, or installed folder.
  </Card>
  <Card title="Empty version" icon="circle-xmark" href="/building-skills/manifest-reference">
    `version` is present but is an empty string.
  </Card>
  <Card title="primary_file not in load_sequence" icon="circle-xmark" href="/building-skills/manifest-reference">
    `primary_file` is declared but the value is not listed in `load_sequence`.
  </Card>
  <Card title="Partial load pair" icon="circle-xmark" href="/building-skills/manifest-reference">
    Exactly one of `primary_file` or `load_sequence` is present. Both must be provided or both must be omitted.
  </Card>
  <Card title="load_sequence / support_files overlap" icon="circle-xmark" href="/building-skills/manifest-reference">
    A path appears in both `load_sequence` and `support_files`.
  </Card>
  <Card title="Unsafe paths" icon="circle-xmark" href="/building-skills/manifest-reference">
    Any declared path is absolute, contains `..`, uses backslashes, contains empty segments, or contains a drive letter.
  </Card>
  <Card title="Script or binary in load_sequence" icon="circle-xmark" href="/building-skills/manifest-reference">
